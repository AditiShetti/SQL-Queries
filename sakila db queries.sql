...
where title = 'Academy Dinosaur' order by r.rental_date desc;
...
-- 28e. Display how much business, in dollars, each store brought in.
 select s.store_id, sum(p.amount) as revenue
 from store s 
  ...
  -- 28f. Display for each store: its store ID, city, and country.
